...
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) GetTotalSelledStaff(c *gin.Context) {
	...
	resp, err := h.storages.Staff().GetByIDTotalSelledStaff(context.Background(), &models.StaffPrimaryKey{StaffId: idInt})
	if err != nil {
		h.handlerResponse(c, "storage.staff.totalSelled", http.StatusInternalServerError, err.Error())
		return
	}

	h.handlerResponse(c, "get staff totalSelled", http.StatusCreated, resp)
}

// Get List Staff godoc
// @ID get_list_staff_selled
// @Router /report/staff-sale [GET]
// @Summary Get List StaffSale
// @Description Get List StaffSale
// @Tags TotalSelledStaff
// @Accept json
// @Produce json
// @Param offset query string false "offset"
...
// @Response 400 {object} Response{data=string} "Bad Request"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) GetListTotalSelledStaff(c *gin.Context) {

	offset, err := h.getOffsetQuery(c.Query("offset"))
	if err != nil {
		h.handlerResponse(c, "get list staffSelled", http.StatusBadRequest, "invalid offset")
		return
	}

	limit, err := h.getLimitQuery(c.Query("limit"))
	if err != nil {
		h.handlerResponse(c, "get list staff selled", http.StatusBadRequest, "invalid limit")
		return
	}

	resp, err := h.storages.Staff().GetListTotalSelledStaff(context.Background(), &models.GetListStaffRequest{
		Offset: offset,
		Limit:  limit,
		Search: c.Query("search"),
	})
	if err != nil {
		h.handlerResponse(c, "storage.staffSelled.getlist", http.StatusInternalServerError, err.Error())
		return
	}

	h.handlerResponse(c, "get list staff Selled response", http.StatusOK, resp)
}
