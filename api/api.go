...
func NewApi(r *gin.Engine, cfg *config.Config, store storage.StorageI, logger logger.LoggerI) {
	...
	r.PUT("/transferProduct", handler.TransferProduct)
	r.GET("/orderTotal/:id", handler.GetOrderTotalSum)
	r.GET("report/staff-sale/:id", handler.GetTotalSelledStaff)
	r.GET("/report/staff-sale", handler.GetListTotalSelledStaff)
	//promoCode api
