...
func (r *stockRepo) TransferProduct(ctx context.Context, req *models.TransferProductRequest) error {
	var (
		query    string
		params   map[string]interface{}
		quantity int
	...
	`
	err := r.db.QueryRow(ctx, query, req.FromStoreID, req.ProductID).Scan(
		&quantity,
	)
	if err != nil {
		log.Println()
	...
	`
		...
		log.Printf("FromStore da yetarlicha mahsulot yoq")
		return errors.New("FromStore da yetarlicha mahsulot yoq")
	}
