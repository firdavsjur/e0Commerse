...
func (r *staffRepo) GetByIDTotalSelledStaff(ctx context.Context, req *models.StaffPrimaryKey) ([]*models.GetTotalSelledStaff, error) {
	var (
		query string
		resp  []*models.GetTotalSelledStaff
	)
	...
	WHERE ord.staff_id =$1 and ord.order_status = 4
	...
	rows, err := r.db.Query(ctx, query, req.StaffId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var staff models.GetTotalSelledStaff
		err = rows.Scan(
			&staff.StaffName,
			&staff.CategoryName,
			&staff.ProductName,
...
func (r *staffRepo) GetListTotalSelledStaff(ctx context.Context, req *models.GetListStaffRequest) (resp []*models.GetTotalSelledStaff, err error) {
